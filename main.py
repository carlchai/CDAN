...
from utils.dataloaders import get_data_loader,get_VL_CMU_CD_Raw,get_pcd_raw
import os
import torch
...
def Train():
    ...
    for epoch in range(args.epochs):
        ...
        metric_logger.add_meter('f1score',
                                metric_util.SmoothedValue(window_size=1, fmt='{value:.4f} ({global_avg:.4f})'))
        with tqdm(total=len(train_dataloader)) as pbar:
            for images, target in train_dataloader:
                ...
                metric_logger.f1score.update(f1score.mean(), n=len(f1score))
                l.append(loss.item())
                pbar.set_postfix(epoch=f'{epoch:.1f}', loss=f'{loss.item():.4f}',
                                 f1score=f'{f1score.mean():.4f}')  # 更新进度条的后缀以显示平均损失
                pbar.update(1)
...
if __name__ == '__main__':
    ...
    args.num_classes = 2
    args.output_dir = 'PCD/raw/output_PCD_c3po_cam'
    ...
    model_path = "PCD/raw/output_PCD_c3po_cam/best.pth"
    save_imgs_dir = "PCD/raw/output_PCD_c3po_cam_img"
    Train()
    save_img(model,model_path,save_imgs_dir,test_dataloader,device)
    #get_GT(model,model_path,save_imgs_dir,"output_img5",test_dataloader,device)
