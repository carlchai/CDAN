...
if __name__ == '__main__':
    ...
    args.num_classes = 2
    args.output_dir = 'PCD/raw/output_PCD_changenet'
    trainset = get_pcd_raw(args, train=True)
    testset = get_pcd_raw(args, train=False)
    train_dataloader = torch.utils.data.DataLoader(trainset,batch_size=batchsize,num_workers=0)
    test_dataloader = torch.utils.data.DataLoader(testset,batch_size=1,num_workers=0)
    #res18 = ResNet18()
    ...
    from models.changenet import model
    model.to(device)
    ...
    Train()
    model_path = "PCD/raw/output_PCD_changenet/best.pth"
    save_imgs_dir = "PCD/raw/output_PCD_changenet_img"
