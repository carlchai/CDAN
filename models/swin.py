...
class SwinTransformer(nn.Module):
    ...
    def Average_filter(self, t1, t2):
        c = t1.shape[2]
        AM = torch.mean(t1 - t2, dim=2, keepdim=True)
        AM = AM.repeat(1,  1, c)
        cm1 = self.relu(t1 - AM)
        cm2 = self.relu(t2 - AM)
        info = cm1 + cm2
    ...
    def forward(self, x):
        ...
        x1, H, W = self.patch_embed(x1)  # 对图像下采样4倍
        x2, H2, W2 = self.patch_embed(x2)  # 对图像下采样4倍
        ...
        for layer in self.layers:
            ...
            info = self.Average_filter(x1,x2)
...
class AF(nn.Module):
    ...
    def forward(self, x):
        ...
        t10, t11 = torch.split(self.x1, 3, dim=1)
        ...
        t20, t21 = torch.split(self.x2, 3, dim=1)
        ...
        t30, t31 = torch.split(self.x3, 3, dim=1)
        ...
        t40, t41 = torch.split(self.x4, 3, dim=1)
...
if __name__ == '__main__':
    ...
    swin = swin_tiny_patch4_window7_224(2).to(device)
    output = swin(tensor)
    print(output.shape)
